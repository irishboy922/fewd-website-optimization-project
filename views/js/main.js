...
var resizePizzas = function(size) {
    ...
    // Created the variable pizzaContainers to store all of the random pizza containers
    // Removed the determinDx function above and combined the sizeSwitcher function with the changePizzaSizes function
    // Refactored the changePizzaSizes code to change the size of the pizza's in a percentage value based off the size selected using the slider
...
function updatePositions() {
    ...
    // Used getElementsByClass, a more efficient way to sellect the mover class elements

    // Took the scroll to top position out of the for loop since it is consistant on all mover elements and declared it in a scrollPos variable
    var scrollPos = document.body.scrollTop / 1250;
    // Declared the length of the items variable in itemsLen
    var itemsLen = items.length;
    // Created an empty phase variable
    var phase;

    for (var i = 0; i < itemsLen; i++) {
        // Gave the phase variable a new random position value based for each item
        phase = Math.sin(scrollPos + (i % 5));
        // Left the original repositioning expression
...
// Put the updatePositions function in a rAF so that all updates are run a single reflow and repaint cycle
...
document.addEventListener('DOMContentLoaded', function() {
    var cols = 8;
    var s = 256;
    // Reduced the number of pizzas from 200 to 30
    for (var i = 0; i < 30; i++) {
        var elem = document.createElement('img');
        elem.className = 'mover';
        elem.src = "images/pizza.png";
        // Removed the height and width and declared these properties in the css stylesheet
